...
import type { RepairOrder } from '../../data/types'
...
export function Repairs() {
  ...
  // 完成
  const [completeRepair, setCompleteRepair] = useState<RepairOrder | null>(null)
  ...
  const repairs = useDbData(() => dataService.listRepairs(role ?? 'contractor', myContractorId))
  ...
  const itemName = (id: number) => {
    ...
    return i ? `${i.item_code} · ${i.name}` : `#${id}`
  }
  const contractorName = (id: number) =>
    contractors.find((c) => c.contractor_id === id)?.name ?? `#${id}`
  ...
  const filtered = useMemo(() => {
    return repairs.filter((r) => {
      if (keyword.trim()) {
        ...
        const hit = item && `${item.item_code} ${item.name}`.toLowerCase().includes(kw)
        if (!hit) return false
      }
      if (filterStatus && r.status !== filterStatus) return false
      if (filterContractor && String(r.contractor_id) !== filterContractor) return false
      return true
    })
  }, [repairs, keyword, filterStatus, filterContractor, items])
  ...
  }, [createForm.contractor_id, createForm.request_date])
  ...
  const handleCreate = () => {
    if (createSubmitting || !role) return
  ...
  const openDetail = (r: RepairOrder) => {
    setDetailRepair(r)
  }

  const handleStart = (r: RepairOrder) => {
    if (!role || myContractorId === undefined) return
    const result = dataService.startRepair(role, myContractorId, r.repair_id)
    if (result.ok) MessagePlugin.success('已开始维修')
    else MessagePlugin.error(result.error)
  }

  const openComplete = (r: RepairOrder) => {
    setCompleteRepair(r)
  ...
  const handleComplete = () => {
    if (completeSubmitting || !role || !completeRepair || myContractorId === undefined) return
  ...
  // 完成维修成本预估（按冻结费率）
  const completeRate = completeRepair
    ? dataService.listContractorRates(completeRepair.contractor_id).find((r) => r.rate_id === completeRepair.rate_id) ?? null
    : null
  const costPreview = completeRate && completeForm.repair_hours
    ? completeRate.hourly_rate * completeForm.repair_hours
    : null

  const columns: PrimaryTableCol<RepairOrder>[] = [
    { colKey: 'no', title: '维修单号', width: 110, className: 'font-mono', cell: ({ row }) => repairNo(row.repair_id) },
    { colKey: 'item', title: '设备', width: 190, ellipsis: true, cell: ({ row }) => itemName(row.item_id) },
    { colKey: 'contractor', title: '承包商', width: 130, cell: ({ row }) => contractorName(row.contractor_id) },
  ...
  const detailRepairData = detailRepair ? dataService.getRepairDetail(detailRepair.repair_id) : null

  return (
    <div>
      <PageHeader title="维修单管理" subtitle="设备报修、承包商接单与完成维修（费率按申请日期冻结）" />
      ...
      <div style={{ background: 'var(--snowpeak-bg-container)', border: '1px solid var(--snowpeak-border)', borderRadius: 8 }}>
        ...
        >
          ...
          {isStaffSide && (
            <Select
              ...
              placeholder="承包商"
              clearable
              options={contractors.map((c) => ({ label: c.name, value: String(c.contractor_id) }))}
              style={{ width: 160 }}
            />
          )}
          <div style={{ flex: 1 }} />
        ...
        <Table
          ...
          hover
          tableLayout="fixed"
          empty={keyword.trim() || filterStatus || filterContractor ? '未找到匹配的维修单' : '暂无维修单'}
          pagination={{
            current: page,
            pageSize,
            total,
        ...
        {detailRepairData && (
          <div style={{ padding: '4px 0' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
              ...
              <StatusTag status={detailRepairData.repair.status} />
            </div>
            <div style={{ fontSize: 13, color: 'var(--snowpeak-text-secondary)', lineHeight: 2 }}>
              <div>设备：{itemName(detailRepairData.repair.item_id)}</div>
              <div>承包商：{detailRepairData.contractor?.name ?? '—'}</div>
              <div>申请日期：{formatDate(detailRepairData.repair.request_date)}</div>
              <div>
                冻结费率：
                {detailRepairData.rate ? (
                  <span className="num">{formatMoney(detailRepairData.rate.hourly_rate)}/小时</span>
                ) : (
                  '—'
                )}
              </div>
              <div>故障描述：{detailRepairData.repair.fault_description}</div>
              {detailRepairData.repair.status === '已完成' && (
                <>
                  <div>维修完成日期：{formatDate(detailRepairData.repair.repair_date as string)}</div>
                  <div>维修工时：{detailRepairData.repair.repair_hours} h</div>
                  <div>
                    维修成本：
                    <span className="num">{formatMoney(detailRepairData.repair.calculated_cost as number)}</span>
                  </div>
                  <div>维修说明：{detailRepairData.repair.notes}</div>
                </>
              )}
            </div>
          </div>
      ...
      >
        <div style={{ padding: '8px 0' }}>
          ...
          <div style={{ background: 'var(--snowpeak-bg-page)', border: '1px solid var(--snowpeak-border)', borderRadius: 6, padding: 12, fontSize: 13 }}>
            {completeRate && completeForm.repair_hours ? (
              <span>
                预计成本：<span className="num">{formatMoney(costPreview as number)}</span>
                （冻结费率 {formatMoney(completeRate.hourly_rate)}/小时 × {completeForm.repair_hours} h）
              </span>
            ) : (
              <span style={{ color: 'var(--snowpeak-text-placeholder)' }}>
                冻结费率 {completeRate ? `${formatMoney(completeRate.hourly_rate)}/小时` : '—'}，输入工时后显示预计成本
              </span>
            )}
          </div>
        </div>
